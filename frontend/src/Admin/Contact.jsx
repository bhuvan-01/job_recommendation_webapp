    ...
    const downloadContactsPDF = () => {
        ...
        doc.autoTable({
            ...
            body: contacts.map((contact, index) => [
                index + 1, contact.name, contact.email, contact.phone, contact.message
            ]),
        });
            ...
            <div className="overflow-x-auto flex-grow">
                <table className="min-w-full bg-white">
                    <thead className="bg-gray-100">
                        <tr>
                            <th className="text-left py-3 px-4 uppercase font-semibold text-sm" style={{ color: 'gray' }}>No.</th>
                            <th className="text-left py-3 px-4 uppercase font-semibold text-sm" style={{ color: 'gray' }}>Name</th>
                            <th className="text-left py-3 px-4 uppercase font-semibold text-sm" style={{ color: 'gray' }}>Email</th>
                            <th className="text-left py-3 px-4 uppercase font-semibold text-sm" style={{ color: 'gray' }}>Phone</th>
                            <th className="text-left py-3 px-4 uppercase font-semibold text-sm" style={{ color: 'gray' }}>Message</th>
                            <th className="text-left py-3 px-4 uppercase font-semibold text-sm" style={{ color: 'gray' }}>Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <div className="mt-3 text-center">
                            ...
                            <div className="mt-2 px-7 py-3">
                                <p className="text-sm text-gray-500">
                                    ...
                                    Email: {selectedContact.email}<br />
                                    Phone: {selectedContact.phone}<br />
                                    Message: {selectedContact.message}
                                </p>
                            </div>
                            <div className="items-center px-4 py-3">
                                <button onClick={() => setSelectedContact(null)} className="px-4 py-2 bg-gray-500 text-white text-base font-medium rounded-md w-full shadow-sm hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-300">
                                    Close
                                </button>
                            </div>
                        </div>
